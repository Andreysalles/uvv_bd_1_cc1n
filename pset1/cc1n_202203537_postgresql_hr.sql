...
-- Prevencao de erro na criacao do usuário
DROP USER IF EXISTS andrey;



-- Criacao do usuario administrador do banco, concede a permissão de criar BD e cadastra a senha para o usuario
CREATE user andrey WITH SUPERUSER password '1234'; 

--Alterando o usuário
ALTER USER andrey;

-- Prevencao de erro na criação do banco de dados
...
-- prevencao de erro na criação do schema
DROP SCHEMA IF EXISTS hr;

-- Criacao do schema
CREATE SCHEMA hr AUTHORIZATION andrey;

-- Colocando o schema hr como default
SET SEARCH_PATH TO hr;

-- Criacao da estrutura do banco, com base no modelo logico do power architect
...
COMMENT ON TABLE cargos IS 'Criação da tabela cargos, contem todos os cargos da empresa, e a faixa salarial (mínimo e máximo) de cada cargo. Possui relacionamento com as tabelas empregados e historico_cargos';
COMMENT ON COLUMN cargos.id_cargo IS 'Criação da PK da tabela cargos.';
COMMENT ON COLUMN cargos.cargo IS 'Criação da coluna cargo da tabela cargos, contem nomes dos cargos.';
COMMENT ON COLUMN cargos.salario_minimo IS 'Criação da coluna salario_minimo da tabela cargos, contem o salário minimo do cargo.';
COMMENT ON COLUMN cargos.salario_maximo IS 'Criação da coluna salario_maximo da tabela cargos, contem o salario maximo do cargo';
...
COMMENT ON TABLE regioes IS 'Criação da tabela regiões, onde contem nome e o id das regiões';
COMMENT ON COLUMN regioes.id_regiao IS 'Criação da PK da tabela regioes';
COMMENT ON COLUMN regioes.nome IS 'Criação da coluna nome, contem o nome das regiões';
...
COMMENT ON TABLE paises IS 'Criação da tabela paises, contem o id do pais e da região, tambem possue o nome dos paises';
COMMENT ON COLUMN paises.id_pais IS 'Criação da PK da tabela paises.';
COMMENT ON COLUMN paises.nome IS 'Criação da coluna nomes, contem o nome dos países.';
COMMENT ON COLUMN paises.id_regiao IS 'Criação da chave estrangeira para tabela regiões.';
...
COMMENT ON TABLE localizacoes IS 'Criação da tabela localização, Contem o edereço, cep, uf e cidade da empresa.';
COMMENT ON COLUMN localizacoes.id_localizacao IS 'Criação da PK da tabela localizações.';
COMMENT ON COLUMN localizacoes.endereco IS 'Criaçãoda coluna endereço da tabela localizaçoes, contem o endereço dos escritórios.';
COMMENT ON COLUMN localizacoes.cep IS 'Criaçãoda coluna cep da tabela localizacoes, contem o cep dos escritórios';
COMMENT ON COLUMN localizacoes.cidade IS 'Criação da coluna cidade da tabela localizacoes, contem as cidades dos escritórios';
COMMENT ON COLUMN localizacoes.uf IS 'Criação da coluna UF da tabela localizações, contem a UF dos escritórios';
COMMENT ON COLUMN localizacoes.id_pais IS 'Criação da FK para tabela países.';
...
COMMENT ON TABLE departamentos IS 'Criação da tabela departamentos, contem informações dos departamentos, essa tabela se relaciona com as tabelas localizações, empregados e historico_cargos';
COMMENT ON COLUMN departamentos.id_departamento IS 'Criação da PK da tabela departamentos.';
COMMENT ON COLUMN departamentos.nome IS 'Criação da coluna nome da tabela departamentos, contem nome dos departamentos.';
COMMENT ON COLUMN departamentos.id_localizacao IS 'Criação da FK para tabela localizações.';
COMMENT ON COLUMN departamentos.id_gerente IS 'Criação da da FK para tabela empregados.';
...
COMMENT ON TABLE empregados IS 'Criação da tabela empregados, se relaciona com as tabelas departamentos, historico_cargos e com ela mesma.';
COMMENT ON COLUMN empregados.id_empregado IS 'Criaçãoda da PK da tabela empregados.';
COMMENT ON COLUMN empregados.nome IS 'Criação da coluna nome da tabela empregados, contem o nome completo do empregado';
COMMENT ON COLUMN empregados.email IS 'Criação da coluna email da tabela empregados, contem a parte inicial do email antes do domínio.';
COMMENT ON COLUMN empregados.telefone IS 'Criação da coluna telefone da tabela empregados, contem o numero de telefone do empregado';
COMMENT ON COLUMN empregados.data_contratacao IS 'Criação da coluna data_contratacao da tabela empregados, contem a data que o empregado foi contratado';
COMMENT ON COLUMN empregados.id_cargo IS 'Criação da FK para tabela cargos.';
COMMENT ON COLUMN empregados.salario IS 'Criação da coluna salario da tabela empregados, possui salário do empregado';
COMMENT ON COLUMN empregados.comissao IS 'Criação da coluna comissao da tabela empregados, Contem porcentagem de comissão de um empregado. Apenas empregados no departamento
de vendas podem receber comissões';
COMMENT ON COLUMN empregados.id_supervisor IS 'Criação da FK id_supervisor que se relaciona com ela mesma,Contem o supervisor direto do empregado.';
COMMENT ON COLUMN empregados.id_departamento IS 'Criação da FK para tabela departamentos. mostra o departamento atual do empregado';
...
CREATE TABLE historico_cargos (
                ...
                id_departamento INTEGER NOT NULL,
                id_cargo VARCHAR(10) NOT NULL,
                CONSTRAINT data_incial PRIMARY KEY (data_inicial, id_empregado)
);
COMMENT ON TABLE historico_cargos IS 'Criação da tabela historico_cargos, possue relacionamento com as tabelas departamentos, cargos e empregados';
COMMENT ON COLUMN historico_cargos.data_inicial IS 'Criação da PK da tabela historico_cargos, contem o histórico de cargos de um empregado. Quando um funcionário muda de cargo ou de departamento, sera adicionado novas linhas sem apagar as antigas, para que fique registrado o cargo antigo.';
COMMENT ON COLUMN historico_cargos.id_empregado IS 'Criaçãoda da PFK para tabela empregados.';
COMMENT ON COLUMN historico_cargos.data_final IS 'Criação da coluna data_final da tabela historico_cargos, contem o ultimo dia do empregado no cargo, terá que ser maior que a data inicial.';
COMMENT ON COLUMN historico_cargos.id_departamento IS 'Criação da PK da tabela departamentos.';
COMMENT ON COLUMN historico_cargos.id_cargo IS 'Criação da FK para tabela cargos.';
...
VALUES (100, 'Steven King', 'SKING', '515.123.4567', '2003-06-17', 'AD_PRES', 24000, null, 120, 90);
INSERT INTO empregados (id_empregado, nome, email, telefone, data_contratacao, id_cargo, salario, comissao, id_supervisor, id_departamento) 
